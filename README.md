...
* **[Build Your Own Redis](https://build-your-own.org/redis/)**
  * 외부 라이브러리 없이 C/C++ 또는 Go로 Redis 핵심 기능을 바닥부터 구현하는 가이드<br>
    Network Socket, Protocol Parsing, Event Loop 구조 학습용
  * `Tags` : `Network` `Event Loop` `C/C++` `Go` `Low-Level`
* **[System Design Notes](https://github.com/liquidslr/system-design-notes)**
  * 대규모 분산 시스템 설계의 주요 아키텍처 패턴 요약본<br>
    Load Balancing, Caching, DB Sharding 등의 핵심 구조 정리
...
* **[Practice C Programming Projects](https://github.com/labex-labs/practice-c-programming-projects)**
  * C 언어 로우레벨 제어 기본기를 다질 수 있는 실습 프로젝트 모음<br>
    Pointer 연산 및 Memory Allocation 등 커스텀 구현 중심
...
* **[Computer Networking Fundamentals](https://labs.iximiuz.com/courses/computer-networking-fundamentals)**
  * 리눅스 가상 환경(Container, Namespace) 기반의 인터랙티브 네트워크 실습 코스<br>
    Routing 및 TCP/IP 흐름 검증 시뮬레이션 인터페이스 제공
...
* **[Math Behind Cross-Entropy Loss](https://outcomeschool.com/blog/math-behind-cross-entropy-loss)**
  * 분류(Classification) 및 대조 학습의 기반이 되는 Loss 함수 분석<br>
    Cross-Entropy Loss를 정보 이론 관점에서 엄밀하게 유도하는 과정 기술
  * `Tags` : `Information Theory` `Loss Function` `Math`
* **[Paper Reading (Mu Li)](https://github.com/mli/paper-reading)**
  * 전 AWS 수석 과학자 Mu Li 박사의 딥러닝 랜드마크 논문 분석 아카이브<br>
    주요 논문의 직관적인 코드 구현 및 논문 리딩 라이브 세션 요약 정리
  * `Tags` : `Deep Learning` `Paper Reading` `Landmark Models`
* **[Transformer Explainer](https://github.com/poloclub/transformer-explainer)**
  * Transformer(GPT-2 기반) 내부의 텐서 연산 과정을 보여주는 시각화 툴<br>
    Self-Attention 및 레이어별 데이터 흐름을 3D 인터랙티브 화면으로 제공
...
* **[Stanford CS229 Machine Learning Cheatsheets](https://github.com/afshinea/stanford-cs-229-machine-learning)**
  * 스탠포드 대학교 CS229 강의의 핵심 요약본 리포지토리<br>
    지도/비지도 학습, 딥러닝 수학 공식 및 확률·선형대수 기초 치트시트 모음
  * `Tags` : `Stanford CS229` `Cheatsheet` `Linear Algebra`
* **[Sophon AI Research Papers](https://sophon.at/papers)**
  * 실시간 트렌딩 AI 연구 논문 및 모델 탐색 플랫폼<br>
    분야별(Language Modeling, Agents 등) 최신 연구 벤치마크 추적용
  * `Tags` : `AI Trend` `Research Paper` `LLM` `Benchmark`

### 🛠 Tools & AI Productivity
* **[Understand Anything](https://github.com/Lum1104/Understand-Anything)**
  * 방대한 코드베이스나 프로젝트 구조를 한눈에 파악하는 오픈소스 툴<br>
    레이어 구조와 함수 종속 관계를 대화형 지식 그래프(Knowledge Graph)로 시각화
...
* **[Andrej Karpathy Skills Guidelines](https://github.com/multica-ai/andrej-karpathy-skills/tree/main)**
  * 안드레아 카파시가 공유한 AI 코딩 피트폴(Pitfalls) 방지 가이드라인<br>
    Claude Code, Cursor 전용 커스텀 룰 규칙 및 프롬프트 팁 포함
  * `Tags` : `Karpathy` `LLM Coding` `Cursor` `Claude Code`
* **[Developer Roadmap](https://github.com/nilbuild/developer-roadmap)**
  * 전 세계 개발자들이 참조하는 roadmap.sh의 공식 저장소<br>
    직무별/기술 스택별 대화형 커리어 및 기술 학습 로드맵 가이드라인
  * `Tags` : `Career` `Roadmap` `Learning Path`

### 📊 Visualization & Utility
* **[Manim Community](https://github.com/ManimCommunity/manim)**
  * 수학적 개념이나 알고리즘 매커니즘을 파이썬 코드로 애니메이션화하는 렌더링 엔진<br>
    3Blue1Brown 스타일의 고품질 교육용 수학 비디오 제작용 오픈소스 프레임워크
  * `Tags` : `Python` `Math Animation` `3blue1brown`
* **[VisuAlgo](https://visualgo.net/en)**
  * 주요 자료구조 및 알고리즘의 동작 과정을 단계별 애니메이션으로 보여주는 플랫폼<br>
    정렬, 트리, 그래프 알고리즘의 데이터 변화 추이를 실시간 트래킹 가능
  * `Tags` : `Data Structure` `Algorithm` `Animation`
* **[CSVistool (ArrayList)](https://csvistool.com/ArrayList)**
  * 동적 배열(ArrayList)의 내부 동작 메커니즘 시각화 도구<br>
    배열 Resize 및 삽입/삭제 시 데이터 시프팅 과정을 직관적으로 표현
  * `Tags` : `Array List` `Data Structure` `CS Basics`
* **[cobalt.tools](https://cobalt.tools)**
  * 광고나 트래커 없이 깔끔하게 미디어 파일만 추출해 주는 웹 유틸리티<br>
    불필요한 리디렉션이나 악성 스크립트 없이 원본 포맷 다운로드 지원
...
* **[Google Engineering Practices](https://github.com/google/eng-practices/blob/master/review/reviewer/comments.md)**
  * 구글의 코드 리뷰 커뮤니케이션 가이드라인 핸드북<br>
    전문적이고 직관적인 리뷰 댓글 작성 템플릿 및 커뮤니케이션 문화 요약 문서
  * `Tags` : `Code Review` `Google` `Engineering Culture`
* **[Academic Phrasebank](https://www.phrasebank.manchester.ac.uk/)**
  * 맨체스터 대학 제공 학술 영어 작문 문장 템플릿 은행<br>
    논문 섹션별(Intro, Methodology 등) 빈출 표현 및 정형화된 문장 구조 정리
  * `Tags` : `Academic Writing` `English` `Research Template`
* **[Write & Improve](https://writeandimprove.com/)**
  * 캠브리지 대학이 개발한 AI 기반 실시간 영작문 교정 서비스<br>
    문법 오류 즉각 검출 및 영어 에세이 자가 학습용 피드백 툴
...
* **[MIT xv6 RISC-V Source & Commentary Book](https://pdos.csail.mit.edu/6.1810/2025/xv6/)**
  * MIT의 OS 강의에서 활용되는 유닉스 기반 교육용 커널 xv6 공식 아카이브 배포처<br>
    `book-riscv-rev5.pdf` 및 `xv6-src-booklet-rev5.pdf` 원본 리소스 공식 수록
...
* **[Andrew Ng's Full AI Prompting Course](https://www.youtube.com/watch?v=8ib4Qnh2HFE)** / **[DeepLearning.AI Official](https://www.deeplearning.ai/courses/ai-prompting-for-everyone)**
  * 앤드류 응(Andrew Ng) 교수가 직강하는 AI 파워유저용 고급 프롬프팅 강의 코스<br>
    맥락 관리, 멀티 에이전트 시스템 파이프라인 설계 가이드 공식 영상 리포지토리
...
* **[Mathematical Foundations of Machine Learning (Seongjai Kim)](https://www.youtube.com/@mathtalent)**
  * 머신러닝 기초부터 모델 수학 공식 유도까지 엄밀하게 다루는 강의 자료 아카이브<br>
    SVM KKT 조건 증명, PCA/SVD 분해 공식 수식 등 `Machine_Learning_Lecture.pdf` 원본 소스처
...
* **[Calculus: Early Transcendentals Web Text](https://open.umn.edu/opentextbooks/textbooks/415)**
  * 미네소타 대학 오픈 텍스트북 라이브러리에 등록된 David Guichard 교수의 미적분학 교재<br>
    머신러닝 경사하강법 등 최적화 수식 연산의 수학적 뼈대를 다지는 오픈 리소스
...
* **[Machine Learning Systems (Harvard Edge / MIT Press)](https://github.com/harvard-edge/cs249r_book)**
  * 하버드 및 MIT 강의에서 쓰이는 임베디드 장치부터 분산 인프라까지 아우르는 ML 시스템 전문 서적<br>
    `Machine-Learning-Systems.pdf` 도서의 공식 퍼블릭 깃허브 저장소 소스 링크
  * `Type` : `Open Book` `MLSys` `Infrastructure`
* **[Breaking the Sorting Barrier for Directed SSSP (arXiv)](https://arxiv.org/abs/2504.17033)**
  * 방향 그래프 최단 경로 알고리즘(SSSP) 분야의 최신 연구 논문 공식 아카이브<br>
    다익스트라 알고리즘의 결정론적 복잡도 장벽 돌파 기법 수록
